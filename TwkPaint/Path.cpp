...
void Path::add(const Point& p)
{
    ...
    if (!m_widths.empty()) m_widths.push_back(m_widths.back());
}
...
static void roundCap(size_t i0, size_t i1, size_t i2, const float dir, PointArray& rPoints,
                     PointArray& rTexCoords, IndexTriangleArray& rTris,
...
void Path::addStamp(const float dir, Point point, float w, Color c, PointArray& rPoints,
                    PointArray& rTexCoords, IndexTriangleArray& rTris,
...
//
void Path::filterPoints(PointArray& fpoints, ScalarArray& directionCoords, ScalarArray& fwidths,
                        ColorArray& fcolors, vector<PointType>& pointTypes, bool splatOnly)
{
    ...
    if (!constWidth) fwidths.push_back(m_widths.front());
    if (hasColor) fcolors.push_back(m_colors.front());
    ...
    for (size_t i = 1; i < m_points.size(); i++)
    {
        ...
        if (splatOnly)
        {
            ...
            if (!constWidth) fwidths.push_back(w1);
            if (hasColor) fcolors.push_back(c1);
        }
        else
        {
            ...
            else
            {
                ...
                if (s >= 3)
                {
                    ...
                    if (pointTypes[s - 3] != SplatPoint && pointTypes[s - 2] != SplatPoint &&
                        pointTypes[s - 1] != SplatPoint && angleBetween(a - b, c - b) < pi / 2.0)
                    {
    ...
    {
        if (to > 0.0) directionCoords[i] /= to;
    }
...
void Path::computeSegmentQuads(SegmentVector& segments, const PointArray& fpoints,
                               const ScalarArray& fwidths, const ColorArray& fcolors,
                               const PointTypeVector& pointTypes, bool constWidth)
{
    ...
    for (size_t i = 0; i < n - 1; i++)
    {
        ...
        if ((type0 == EndPoint && type1 == StartPoint) || type0 == SplatPoint ||
            type1 == SplatPoint)
        {
            s.none = true;
            continue;
...
void Path::computeJoins(JoinVector& joins, const SegmentVector& segments, const PointArray& fpoints,
                        const ScalarArray& fwidths, const ColorArray& colors,
                        const PointTypeVector& pointTypes, bool constWidth)
{
    ...
    for (size_t i = 0; i < n; i++)
    {
        ...
        {
            if (usecolor) j.colors[0] = colors[i];
        }
        ...
        else if (j.type == BodyPoint)
        {
            ...
            else if (c.z > 0)
            {
                ...
                else
                {
                    ...
                    j.miterPoint = intersectionOfLines(Vec2d(s0.points[1]), Vec2d(s0.points[2]),
            ...
            else // if (c.z < 0)
            {
                ...
                {
                    j.miterPoint = intersectionOfLines(Vec2d(s0.points[0]), Vec2d(s0.points[3]),
...
void Path::buildSegmentGeometry(const Segment& segment, Join& j0, const float dir0, Join& j1,
                                const float dir1, PointArray& rPoints, PointArray& rTexCoords,
...
void Path::buildJoin(JoinStyle js, const Join& j, const float dir, PointArray& rPoints,
                     PointArray& rTexCoords, IndexTriangleArray& rTris,
                     ScalarArray& rDirectionCoords)
{
    if (j.type == SplatPoint)
    {
        addStamp(dir, j.center, j.width, j.colors[0], rPoints, rTexCoords, rTris, rDirectionCoords,
                 m_rColors);
        return;
    }
...
void Path::buildCap(CapStyle cs, const Join& j, const float directionality, PointArray& rPoints,
                    PointArray& rTexCoords, IndexTriangleArray& rTris,
                    ScalarArray& rDirectionCoords)
{
    switch (cs)
    ...
    case RoundCap:
        ...
        {
            roundCap(j.indices[2], j.indices[0], j.indices[1], directionality, rPoints, rTexCoords,
                     rTris, rDirectionCoords);
        }
        else if (j.type == EndPoint)
        {
            roundCap(j.indices[2], j.indices[1], j.indices[0], directionality, rPoints, rTexCoords,
                     rTris, rDirectionCoords);
        }
        break;
    }
...
void Path::computeGeometry(JoinStyle js, CapStyle cs, TextureStyle ts, Algorithm algo,
                           bool dosmooth, float smoothInterval, bool splatOnly)
{
    if (dosmooth) smooth(smoothInterval);
    ...
    if (n == 1)
    {
        ...
        addStamp(0, fpoints.front(), constWidth ? m_width : fwidths.front(),
                 hasColor ? fcolors.front() : black, m_rPoints, m_rTexCoords, m_rTris,
                 m_rDirectionCoords, m_rColors);
        return;
    }
    ...
    for (size_t i = 0; i < n - 1; i++)
    {
        ...
        buildSegmentGeometry(s, joins[i], diri, joins[i + 1], diriplus1, m_rPoints, m_rTexCoords,
    ...
    for (size_t i = 0; i < joins.size(); i++)
    {
        ...
        {
            buildJoin(js, j, m_directionCoords[i], m_rPoints, m_rTexCoords, m_rTris,
                      m_rDirectionCoords);
        }
    }
    ...
    for (size_t i = 0; i < joins.size(); i++)
    {
        ...
        if (j.type != StartPoint && j.type != EndPoint) continue;
...
void Path::dumpOBJ()
{
    ...
    for (size_t i = 0; i < m_rTris.size(); i++)
    {
        ...
        cout << "f " << t[0] << "/" << t[0] << " " << t[1] << "/" << t[1] << " " << t[2] << "/"
             << t[2] << endl;
    }
}

void Path::sampleCentripetalCatmull(const Point p0, const Point p1, const Point p2, const Point p3,
                                    size_t n, PointArray& newPoints)
{
    ...
    t2 = t1 +
         pow((Scalar)((p2.x - p1.x) * (p2.x - p1.x) + (p2.y - p1.y) * (p2.y - p1.y)), (Scalar)0.25);
    t3 = t2 +
         pow((Scalar)((p3.x - p2.x) * (p3.x - p2.x) + (p3.y - p2.y) * (p3.y - p2.y)), (Scalar)0.25);
...
void Path::smooth(float distmult)
{
    ...
    if (!constWidth) inWidths.push_back(m_widths.front());
    if (hasColor) inColors.push_back(m_colors.front());
    ...
    for (size_t i = 1; i < m_points.size(); i++)
    {
        ...
        if (d > Math<Scalar>::epsilon())
        {
            ...
            if (!constWidth) inWidths.push_back(m_widths[i]);
            if (hasColor) inColors.push_back(m_colors[i]);
        }
    }

    inPoints.back() = m_points.back();
    if (!constWidth) inWidths.back() = m_widths.back();
    if (hasColor) inColors.back() = m_colors.back();

    if (inPoints.size() < 3) return;

    newPoints.push_back(inPoints.front());
    if (!constWidth) newWidths.push_back(inWidths.front());
    if (hasColor) newColors.push_back(inColors.front());
    ...
    for (size_t i = 1; i < inPoints.size(); i++)
    {
        ...
        if (n > 1)
        {
            ...
            for (size_t q = 1; q < n; q++)
            {
                ...
                if (!constWidth) newWidths.push_back(lerp<Scalar>(w0, w3, t));
                if (hasColor) newColors.push_back(lerp<Color>(c0, c3, t));
            }

            newPoints.push_back(p3);
            if (!constWidth) newWidths.push_back(w3);
            if (hasColor) newColors.push_back(c3);
        }
        else
        {
            newPoints.push_back(inPoints[i]);
            if (!constWidth) newWidths.push_back(inWidths[i]);
            if (hasColor) newColors.push_back(inColors[i]);
        }
    }
