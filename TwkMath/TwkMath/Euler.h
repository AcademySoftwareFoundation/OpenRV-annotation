...
template <typename T> Mat44<T> Euler<T>::matrix44() const
{
    ...
    if (repeating())
    {
        return Mat4(cj, sj * si, sj * ci, T(0), sj * sh, -cj * ss + cc, -cj * cs - sc, T(0),
                    -sj * ch, cj * sc * cs, cj * cc - ss, T(0), T(0), T(0), T(0), T(1));
    }
    else
    {
        return Mat4(cj * ch, sj * sc - cs, sj * cc + ss, T(0), cj * sh, sj * ss + cc, sj * cs - sc,
                    T(0), -sj, cj * si, cj * ci, T(0), T(0), T(0), T(0), T(1));
    }
}
...
template <class T> T Euler<T>::angleMod(T angle)
{
    ...
    if (angle < -Math<T>::pi()) angle += 2 * Math<T>::pi();
    if (angle > +Math<T>::pi()) angle -= 2 * Math<T>::pi();
