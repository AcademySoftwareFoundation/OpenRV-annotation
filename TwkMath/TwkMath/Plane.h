...
template <typename T> inline bool Plane<T>::less(const Point& a, const Point& b)
{
    return a.z < b.z || (a.z == b.z && (a.y < b.y || (a.y == b.y && a.x < b.x)));
}

template <typename T>
inline void Plane<T>::set(const typename Plane<T>::Point& p, const typename Plane<T>::Vec& v)
{
    normal   = v.normalized();
...
template <typename T>
inline Plane<T>::Plane(const typename Plane<T>::Point& p, const typename Plane<T>::Vec& v)
{
    set(p, v);
}
...
template <typename T>
inline Plane<T>::Plane(const typename Plane<T>::Point& a, const typename Plane<T>::Point& b,
                       const typename Plane<T>::Point& c)
{
    set(a, b, c);
}

template <typename T> inline T Plane<T>::distanceTo(const typename Plane<T>::Point& p) const
{
    return dot((p - normal * distance), normal);
}

template <typename T>
typename Plane<T>::Point Plane<T>::reflect(const typename Plane<T>::Point& p) const
{
    return -2.0 * distanceTo(p) * normal + p;
}

template <typename T>
inline typename Plane<T>::Point Plane<T>::nearestPointTo(const typename Plane<T>::Point& p) const
{
    return -distanceTo(p) * normal + p;
}

template <typename T> inline T Plane<T>::intersectionU(const typename Plane<T>::LineT& line) const
{
...
template <typename T> inline bool Plane<T>::parallel(const typename Plane<T>::LineT& line) const
{
    return dot(normal, line.direction) == T(0);
}

template <typename T> inline bool Plane<T>::parallel(const Plane<T>& plane) const
{
    //
...
template <typename T>
inline typename Plane<T>::Point Plane<T>::intersection(const typename Plane<T>::LineT& line) const
{
...
template <typename T>
inline typename Plane<T>::LineT Plane<T>::intersection(const Plane<T>& plane) const
{
    const T r  = dot(plane.normal, normal);
    const T d  = T(-1) / (r * r);
    const T c0 = (distance - r * plane.distance) * d;
    const T c1 = (plane.distance - r * distance) * d;
    return LineT(c0 * normal + c1 * plane.normal, cross(plane.normal, normal));
}

template <typename T> inline bool operator==(const Plane<T>& a, const Plane<T>& b)
{
    if (a.normal != b.normal) return false;
    const T r = a.distance - b.distance;
    if (Math<T>::abs(r) < Math<T>::epsilon()) return true;
    return false;
}
