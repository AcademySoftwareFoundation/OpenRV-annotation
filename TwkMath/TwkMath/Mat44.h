...
template <typename T> class Mat44
{
  public:
    ...
    Mat44(const T& v00, const T& v01, const T& v02, const T& v03, const T& v10, const T& v11,
    ...
    template <typename S>
    void set(const S& v00, const S& v01, const S& v02, const S& v03, const S& v10, const S& v11,
             const S& v12, const S& v13, const S& v20, const S& v21, const S& v22, const S& v23,
    ...
    bool isAffine() const { return ((m33 == T1) && (m32 == T0) && (m31 == T0) && (m20 == T0)); }
    ...
    template <typename S> void makeRotation(const Vec3<S>& axis, const T& angleInRadians);
...
template <typename T>
inline const T* Mat44<T>::operator[](typename Mat44<T>::size_type rowIndex) const
{
    assert(rowIndex < 4);
#ifdef COMPILER_GCC2
...
template <typename T> inline T* Mat44<T>::operator[](typename Mat44<T>::size_type rowIndex)
{
    assert(rowIndex < 4);
#ifdef COMPILER_GCC2
...
template <typename T>
inline const T& Mat44<T>::operator()(typename Mat44<T>::size_type rowIndex,
...
{
    return Mat44<T>(m00, m10, m20, m30, m01, m11, m21, m31, m02, m12, m22, m32, m03, m13, m23, m33);
}

template <typename T> T Mat44<T>::determinant() const
{
    return determinant4x4(m00, m01, m02, m03, m10, m11, m12, m13, m20, m21, m22, m23, m30, m31, m32,
                          m33);
}
...
template <typename T> template <typename S> Vec3<S> Mat44<T>::transform(const Vec3<S>& v) const
{
    if (isAffine())
    {
...
{
    // assert( isAffine() );
    return Vec3<S>((m00 * d.x) + (m01 * d.y) + (m02 * d.z), (m10 * d.x) + (m11 * d.y) + (m12 * d.z),
                   (m20 * d.x) + (m21 * d.y) + (m22 * d.z));
}

template <typename T> template <typename S> inline Mat44<T>& Mat44<T>::operator+=(const Mat44<S>& v)
{
    m00 += v.m00;
    m01 += v.m01;
    m02 += v.m02;
...
template <typename T> template <typename S> inline Mat44<T>& Mat44<T>::operator*=(const S& v)
{
    m00 *= v;
    m01 *= v;
    m02 *= v;
...
template <typename T> template <typename S> Mat44<T>& Mat44<T>::operator*=(const Mat44<S>& v)
{
    const T v00 = (m00 * v.m00) + (m01 * v.m10) + (m02 * v.m20) + (m03 * v.m30);
    const T v01 = (m00 * v.m01) + (m01 * v.m11) + (m02 * v.m21) + (m03 * v.m31);
    const T v02 = (m00 * v.m02) + (m01 * v.m12) + (m02 * v.m22) + (m03 * v.m32);
    const T v03 = (m00 * v.m03) + (m01 * v.m13) + (m02 * v.m23) + (m03 * v.m33);

    const T v10 = (m10 * v.m00) + (m11 * v.m10) + (m12 * v.m20) + (m13 * v.m30);
    const T v11 = (m10 * v.m01) + (m11 * v.m11) + (m12 * v.m21) + (m13 * v.m31);
    const T v12 = (m10 * v.m02) + (m11 * v.m12) + (m12 * v.m22) + (m13 * v.m32);
    const T v13 = (m10 * v.m03) + (m11 * v.m13) + (m12 * v.m23) + (m13 * v.m33);

    const T v20 = (m20 * v.m00) + (m21 * v.m10) + (m22 * v.m20) + (m23 * v.m30);
    const T v21 = (m20 * v.m01) + (m21 * v.m11) + (m22 * v.m21) + (m23 * v.m31);
    const T v22 = (m20 * v.m02) + (m21 * v.m12) + (m22 * v.m22) + (m23 * v.m32);
    const T v23 = (m20 * v.m03) + (m21 * v.m13) + (m22 * v.m23) + (m23 * v.m33);

    const T v30 = (m30 * v.m00) + (m31 * v.m10) + (m32 * v.m20) + (m33 * v.m30);
    const T v31 = (m30 * v.m01) + (m31 * v.m11) + (m32 * v.m21) + (m33 * v.m31);
    const T v32 = (m30 * v.m02) + (m31 * v.m12) + (m32 * v.m22) + (m33 * v.m32);
    const T v33 = (m30 * v.m03) + (m31 * v.m13) + (m32 * v.m23) + (m33 * v.m33);
...
template <typename T> inline typename Mat44<T>::size_type rowDimension(const Mat44<T>&)
{
    return 4;
}

template <typename T> inline typename Mat44<T>::size_type colDimension(const Mat44<T>&)
{
    return 4;
}
...
template <typename T> inline void transpose(Mat44<T>& m)
{
    m.transpose();
}
...
template <typename T, typename S> inline void makeTranslation(Mat44<T>& m, const Vec3<S>& t)
{
    m.makeTranslation(t);
}

template <typename T, typename S> inline Mat44<T> translationMatrix(const Vec3<S>& t)
{
    Mat44<T> ret((void*)1);
...
template <typename T, typename S> inline void setTranslation(Mat44<T>& m, const Vec3<S>& t)
{
    m.setTranslation(t);
}

template <typename T, typename S> inline void addTranslation(Mat44<T>& m, const Vec3<S>& t)
{
    m.addTranslation(t);
}

template <typename T, typename S> inline void makeScale(Mat44<T>& m, const Vec3<S>& s)
{
    m.makeScale(s);
}

template <typename T, typename S> inline Mat44<T> scaleMatrix(const Vec3<S>& s)
{
    Mat44<T> ret((void*)1);
    ret.makeScale(s);
    return ret;
...
inline void makeRotation(Mat44<T>& m, const Vec3<S>& axis, const T& angleInRadians)
{
...
template <typename T, typename S> inline Vec3<T> transform(const Mat44<T>& m, const Vec3<S>& v)
{
    return m.transform(v);
}

template <typename T, typename S> inline Vec3<T> transformDir(const Mat44<T>& m, const Vec3<S>& v)
{
...
template <typename T> inline Mat44<T> operator+(const Mat44<T>& a, const Mat44<T>& b)
{
    return Mat44<T>(a.m00 + b.m00, a.m01 + b.m01, a.m02 + b.m02, a.m03 + b.m03,
...
{
    return Mat44<T>(-a.m00, -a.m01, -a.m02, -a.m03, -a.m10, -a.m11, -a.m12, -a.m13, -a.m20, -a.m21,
                    -a.m22, -a.m23, -a.m30, -a.m31, -a.m32, -a.m33);
}
...
template <typename T, typename S> inline Mat44<T> operator*(const Mat44<T>& a, const S& b)
{
    return Mat44<T>(a.m00 * b, a.m01 * b, a.m02 * b, a.m03 * b, a.m10 * b, a.m11 * b, a.m12 * b,
                    a.m13 * b, a.m20 * b, a.m21 * b, a.m22 * b, a.m23 * b, a.m30 * b, a.m31 * b,
                    a.m32 * b, a.m33 * b);
}

template <typename T, typename S> inline Mat44<T> operator*(const S& a, const Mat44<T>& b)
{
    return Mat44<T>(a * b.m00, a * b.m01, a * b.m02, a * b.m03, a * b.m10, a * b.m11, a * b.m12,
                    a * b.m13, a * b.m20, a * b.m21, a * b.m22, a * b.m23, a * b.m30, a * b.m31,
                    a * b.m32, a * b.m33);
}
...
template <typename T, typename S> inline Vec4<S> operator*(const Vec4<S>& v, const Mat44<T>& m)
{
    return Vec4<S>((v.x * m.m00) + (v.y * m.m10) + (v.z * m.m20) + (v.w * m.m30),
                   (v.x * m.m01) + (v.y * m.m11) + (v.z * m.m21) + (v.w * m.m31),
                   (v.x * m.m02) + (v.y * m.m12) + (v.z * m.m22) + (v.w * m.m32),
                   (v.x * m.m03) + (v.y * m.m13) + (v.z * m.m23) + (v.w * m.m33));
}

template <typename T, typename S> inline Vec4<S> operator*(const Mat44<T>& m, const Vec4<S>& v)
{
    return Vec4<S>((m.m00 * v.x) + (m.m01 * v.y) + (m.m02 * v.z) + (m.m03 * v.w),
...
// Either direction.
template <typename T, typename S> inline Vec3<S> operator*(const Vec3<S>& v, const Mat44<T>& m)
{
...
template <typename T, typename S> inline Vec3<S> operator*(const Mat44<T>& m, const Vec3<S>& v)
{
    Vec4<S> v4(v.x, v.y, v.z, T1);
    v4 = m * v4;
    return Vec3<S>(v4.x / v4.w, v4.y / v4.w, v4.z / v4.w);
...
template <typename T> inline Mat44<T> operator*(const Mat44<T>& a, const Mat44<T>& b)
{
    return Mat44<T>((a.m00 * b.m00) + (a.m01 * b.m10) + (a.m02 * b.m20) + (a.m03 * b.m30),
...
template <typename T> inline bool operator==(const Mat44<T>& a, const Mat44<T>& b)
{
