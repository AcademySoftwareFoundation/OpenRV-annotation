...
template <typename T> inline RigidTransform<T>::operator Mat44<T>() const
{
    ...
    return Mat44<T>(a * scale.x, d * scale.y, g * scale.z, translation.x, b * scale.x, e * scale.y,
                    h * scale.z, translation.y, c * scale.x, f * scale.y, i * scale.z,
                    translation.z, T(0), T(0), T(0), T(1));
}

template <typename T, typename InterpT>
inline RigidTransform<T> lerp(const RigidTransform<T>& a, const RigidTransform<T>& b,
                              const InterpT u)
{
    const InterpT u0 = InterpT(1) - u;
    return RigidTransform<T>(a.translation * u0 + b.translation * u,
                             slerp(a.orientation, b.orientation, u), a.scale * u0 + b.scale * u);
}
