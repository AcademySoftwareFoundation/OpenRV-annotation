...
template <typename T> inline bool equalTol(const T& a, const T& b, const T& tol)
{
    return (Math<T>::abs(a - b) < tol);
}

template <typename T>
inline T areaOfTriangle(const Vec3<T>& va, const Vec3<T>& vb, const Vec3<T>& vc)
{
    Vec3<T> v0 = vb - va;
    Vec3<T> v1 = vc - va;
...
template <typename T>
Vec2<T> intersectionOfLines(const Vec2<T>& a, const Vec2<T>& b, const Vec2<T>& c, const Vec2<T>& d)
{
    const T x1 = a.x;
    const T x2 = b.x;
    const T x3 = c.x;
