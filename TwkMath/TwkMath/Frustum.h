...
template <typename T> class Frustum
{
  public:
    ...
    Frustum(value_type left, value_type right, value_type bottom, value_type top,
            value_type nearPlane, value_type farPlane, bool ortho = false);
    ...
    void window(value_type left, value_type right, value_type bottom, value_type top,
                value_type nearPlane, value_type farPlane, bool ortho = false);
    ...
    void perspective(value_type fovyRadians, value_type aspect, value_type nearPlane,
                     value_type farPlane);
    ...
    void RIBperspective(value_type RIBfovRadians, value_type aspect, value_type nearPlane,
                        value_type farPlane);
...
template <typename T>
inline Frustum<T>::Frustum(T left, T right, T bottom, T top, T nearPlane, T farPlane, bool ortho)
{
...
template <typename T>
inline void Frustum<T>::window(T left, T right, T bottom, T top, T nearPlane, T farPlane,
                               bool ortho)
{
    m_left   = left;
    m_right  = right;
...
template <typename T>
inline void Frustum<T>::RIBperspective(T fov, T aspect, T nearPlane, T farPlane)
{
...
template <typename T> Mat44<T> Frustum<T>::matrix() const
{
    ...
    if (m_ortho)
    {
        ...
        return matrix_type(A, 0, 0, -rpl / rml, 0, B, 0, -tpb / tmb, 0, 0, C, -fpn / fmn, 0, 0, 0,
                           T(1));
    }
    else
    {
        ...
        return matrix_type(n2 / rml, 0, rpl / rml, 0, 0, n2 / tmb, tpb / tmb, 0, 0, 0, -fpn / fmn,
