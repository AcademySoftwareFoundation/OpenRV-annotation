...
template <typename T, typename S> inline Quat<T> operator*(const Quat<T>& a, S b)
{
...
template <typename T> inline void Quat<T>::invert()
{
    *this = inverted();
}
...
template <typename T>
inline void Quat<T>::setAxisAngle(const typename Quat<T>::Vec& axis, T radians)
{
...
template <typename T, typename S> inline T dot(const Quat<T>& a, const Quat<S>& b)
{
