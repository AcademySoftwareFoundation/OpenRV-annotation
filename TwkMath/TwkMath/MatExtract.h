...
template <typename T> inline Mat44<T> xRotationMatrix(const T& xRot)
{
    ...
    return Mat44<T>(T(1), T(0), T(0), T(0), T(0), cosr, -sinr, T(0), T(0), sinr, cosr, T(0), T(0),
                    T(0), T(0), T(1));
}
...
template <typename T> void extractEulerXYZ(const Mat44<T>& mat, Vec3<T>& rot)
{
    ...
    Mat44<T> M(i[0], j[0], k[0], T(0), i[1], j[1], k[1], T(0), i[2], j[2], k[2], T(0), T(0), T(0),
               T(0), T(1));
...
//******************************************************************************
template <typename T>
bool checkForZeroScaleInCol(const T& scl, const Vec3<T>& col, bool throwOnError)
{
    for (int i = 0; i < 3; ++i)
    {
...
//******************************************************************************
template <typename T>
bool extractAndRemoveScalingAndShear(Mat44<T>& mat, Vec3<T>& scl, Vec3<T>& shr, bool throwOnError)
{
    Vec3<T> col[3];
...
//******************************************************************************
template <typename T>
bool extractSHRT(const Mat44<T>& mat, Vec3<T>& Scale, Vec3<T>& sHear, Vec3<T>& Rotation,
                 Vec3<T>& Translation, bool THROW_ON_ERROR)
{
    Mat44<T> rot = mat;
...
template <typename T> Mat44<T> alignZAxisWithTargetDir(Vec3<T> targetDir, Vec3<T> upDir)
{
    ...
    return Mat44<T>(col[0][0], col[1][0], col[2][0], T(0), col[0][1], col[1][1], col[2][1], T(0),
                    col[0][2], col[1][2], col[2][2], T(0), T(0), T(0), T(0), T(1));
}

//******************************************************************************
template <typename T>
Mat44<T> rotationMatrixWithUpDir(const Vec3<T>& fromDir, const Vec3<T>& toDir, const Vec3<T>& upDir)
{
