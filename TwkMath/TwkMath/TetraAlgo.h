...
template <typename T>
inline T volumeOfTetra(const Vec3<T>& va, const Vec3<T>& vb, const Vec3<T>& vc, const Vec3<T>& vd)
{
    return Math<T>::abs(areaOfTriangle(va, vb, vc) * elevation(va, vb, vc, vd) / T(3));
}

template <typename T>
inline bool insideTetra(const Vec3<T>& a, const Vec3<T>& b, const Vec3<T>& c, const Vec3<T>& d,
                        const Vec3<T>& p)
{
    return (elevation(d, c, b, p) <= T(0) && elevation(c, d, a, p) <= T(0) &&
            elevation(b, a, d, p) <= T(0) && elevation(a, b, c, p) <= T(0));
}
...
Vec3<T> randomPointInTetra(const Vec3<T>& v0, const Vec3<T>& v1, const Vec3<T>& v2,
                           const Vec3<T>& v3, RANDGEN& randGen)
{
