...
template <typename T>
Mat44<T> multiplyAsHomogeneous(const std::vector<Vec3<T>>& A, const std::vector<Vec3<T>>& B)
{
    Mat44<T> M;
...
template <typename T>
Mat44<T> LeastSquaredFitTransform(const std::vector<Vec3<T>>& A, const std::vector<Vec3<T>>& B)
{
    //       T   -1
    // B A (A  A)   == M
    //
...
template <typename T> Frustum<T> DecomposeOrthoProjection(const Mat44<T>& P)
{
    ...
    return Frustum<T>(-((g + T(1)) / a), -((g - T(1)) / a), -((d + T(1)) / c), -((d - T(1)) / c),
