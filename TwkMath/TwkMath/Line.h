...
template <class T>
inline Line<T>::Line(const typename Line<T>::Point& p1, const typename Line<T>::Dir& d)
{
    origin    = p1;
...
template <class T>
inline typename T::value_type Line<T>::nearestUTo(const typename Line<T>::Point& p) const
{
...
template <class T> inline T Line<T>::nearestPointTo(const Line<T>& line) const
{
