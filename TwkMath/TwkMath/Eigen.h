...
inline void sampleMean(const ARRAY2D& samples, int numSamples, int dimension, ARRAY1D& mean,
                       T dummy)
{
    // Set mean to zero.
    int dim;
...
// scalars.
template <class ARRAY2D, class ARRAY1D, class T>
inline void sampleMean(const ARRAY2D& samples, int numSamples, ARRAY1D& mean, T dummy)
{
    // Set mean to zero.
    mean = (T)0;
...
void sampleCovarianceMatrix(const ARRAY2D_1& samples, int numSamples, int dimension,
                            const ARRAY1D& mean, ARRAY2D_2& covarMat, T dummy)
{
    // Counter variables
    int samp;
    int row;
    ...
    for (samp = 0; samp < numSamples; samp++)
    {
        ...
        for (row = 0; row < dimension; row++)
        {
            ...
            {
                covarMat[row][col] +=
                    ((T)(samples[samp][row] - mean[row])) * ((T)(samples[samp][col] - mean[col]));
            }
        }
    }
            ...
            for (iq = ip + 1; iq < N; iq++)
            {
                ...
                else if (Math<T>::abs(a[ip][iq]) > threshold)
                {
                    ...
                    else
                    {
                        ...
                        t     = ((T)1) /
                            (Math<T>::abs(theta) + Math<T>::sqrt(((T)1) + (theta * theta)));
                        if (theta < (T)0)
                        {
                            t = -t;
                        }
...
{
    bool operator()(const IndexValuePair<T>& lhs, const IndexValuePair<T>& rhs) const
    {
